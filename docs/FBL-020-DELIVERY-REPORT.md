...
| `064_sales_pipeline_showroom.sql`              | `2fb54e127f479d2f200c4a3d36d16f6cc6b2f536` | `41175a23729861546b33fb1f130a0db4f574a843224d8500c36d3959c2dbb74e` |
...
Sales Pipeline and Showroom Management), not part of this delivery.** It is listed here
for the same reason `061`, `062` and `063` are: this table is the one census of the
migration chain on disk. Over the immutable `000`–`063` it adds the opportunity received
from a CRM handoff and the enumerated stage machine it moves through, the showroom floor —
the up-rotation, the visits standing on it and their event history — the shortlist of cars
a customer is considering, test drives, the sales-side activity timeline, negotiation
rounds and manager turnovers: eleven tables, every one row-secured, registering
...
**IT CARRIES NO MONEY, AND THAT IS ENFORCED RATHER THAN INTENDED.** A dealership's gross,
its commission and its closing ratio are desking work (FBL-120), and none of it exists
yet. Three things keep the promise structurally rather than by convention: the named
constraints `ck_opportunity_pre_deal` and `ck_negotiation_pre_desking` refuse any row
declaring a deal or a price AVAILABLE; `negotiation_rounds` has no money COLUMN at all, so
there is nothing to fill in by accident; and `ck_attribution_pre_sale_revenue` — Release
Train 3's — is untouched and still standing, which `tests/sales-authority.test.ts` asserts
directly. The console says the same thing in words rather than showing a zero somebody
would read as "we made nothing".

Two invariants in it are decided by the DATABASE rather than by a check in the service,
because both are races a real Saturday produces: `uq_demonstrations_vehicle_out` makes it
impossible for one car to be out on two test drives, and the floor rotation is taken under
`FOR UPDATE SKIP LOCKED` so two managers greeting two walk-ins get two different
salespeople. `tests/sales-floor.test.ts` runs both races rather than reasoning about them,
and proves the second demonstration genuinely BLOCKS by asserting on `pg_locks` rather than
by waiting a fixed number of milliseconds. Its batteries are `tests/sales-journey.test.ts`,
`tests/sales-floor.test.ts`, `tests/sales-authority.test.ts` and
`tests/sales-isolation.test.ts` — the last of which proves row security and its policy on
every one of the eleven tables through a genuine `dealership_app` connection rather than
the pooled owner one, because 064 does not FORCE row security and the owner bypasses it.
The fixture-chain declaration (`negative-control-057`) pins its digest.

**FIVE DEFECTS IN THIS TRAIN WERE FOUND BY ITS OWN PROBES RATHER THAN BY READING**, and
they are recorded because a train that reports only what it built teaches nothing about
what nearly shipped. (1) A manager naming a salesperson by hand was checked for reaching
the rooftop but never for being ON the floor — so a customer could be handed to somebody
who had gone home, and because the override skipped the rotation write the up-list would
have given that person a second walk-in. (2) Starting a test drive was a read-then-insert
with no lock: `uq_demonstrations_vehicle_out` made two simultaneous drives impossible, but
the loser got a raw constraint violation as a 500 instead of being told which drive had the
car. (3) `GET /api/sales/floor` picked a rooftop for a caller who could reach several,
answering a question nobody asked with an answer that changed with binding order. (4) The
three commands that CREATE a row — joining the floor, leaving it, recording an arrival —
name no `resourceType`, so their rooftop arrived as a plain field in the request body that
no service checked; through HTTP the policy engine's scope hint already refused a
rooftop-bound caller, but a worker or script calling the services directly was not refused
at all. (5) Eight enumerated fields were passed straight to their CHECK constraints, so a
caller's typo came back as "Internal Server Error" rather than as the list of words the
platform actually uses. Each is fixed, each has a probe that fails without the fix, and
(1), (2), (4) and (5) are in the mutation registry.
...
| `scripts/parse-test-summary.ts`              | declared floors **829 / 83**, above every minimum below                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `scripts/mutation-kill.ts`                   | **74 declared / 74 killed / 0 survived** — a COMPLETE run of the whole registry, every baseline green, the working tree intact after each. The artifact records WHEN it was taken and WHAT it measured in its own `generated_at`, `head` and `tree_dirty` fields; an earlier run that came back 43 / 1 on a red baseline is disclosed in §2.2 (S6a) rather than dropped                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `scripts/database-control-mutations.ts`      | **<!--fig:db_controls_declared-->72<!--/fig--> declared / <!--fig:db_controls_killed-->72<!--/fig--> killed / <!--fig:db_controls_survived-->0<!--/fig--> survived** — each §3 database control DROPPED from a copy of the migrated database, a NAMED test required to die, then restored and required to pass again (§6.1). `controls_filtered` is `false`, so a narrowed diagnostic run cannot stand in for the gate                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
**74 declared / 74 killed / 0 survived, 0 baselines red, 0 trees left dirty.** A report that
...
| `scripts/parse-test-summary.ts` | <!--fig:floor_tests-->829<!--/fig--> / <!--fig:floor_suites-->83<!--/fig-->                       | the DECLARED floor, pinned to what this revision actually measured                      |
...
| ---------------------------- | --------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `suite_tests`                | <!--fig:suite_tests-->829<!--/fig-->                                                                      | `artifacts/test-summary.json` `tests`                                               |
| `suite_suites`               | <!--fig:suite_suites-->83<!--/fig-->                                                                      | `artifacts/test-summary.json` `suites`                                              |
| `suite_passed`               | <!--fig:suite_passed-->829<!--/fig-->                                                                     | `artifacts/test-summary.json` `passed`                                              |
| `observed_ok`                | <!--fig:observed_ok-->912<!--/fig-->                                                                      | `artifacts/test-summary.json` `observed_ok_lines`                                   |
| `observed_not_ok`            | <!--fig:observed_not_ok-->0<!--/fig-->                                                                    | `artifacts/test-summary.json` `observed_not_ok_lines`                               |
| `floor_tests`                | <!--fig:floor_tests-->829<!--/fig-->                                                                      | `scripts/parse-test-summary.ts` `MINIMUM_TESTS`                                     |
| `floor_suites`               | <!--fig:floor_suites-->83<!--/fig-->                                                                      | `scripts/parse-test-summary.ts` `MINIMUM_SUITES`                                    |
...
| `mutations_declared`         | <!--fig:mutations_declared-->74<!--/fig-->                                                                | `artifacts/mutation-kill.json` `mutations_total`                                    |
| `mutations_killed`           | <!--fig:mutations_killed-->74<!--/fig-->                                                                  | `artifacts/mutation-kill.json` `mutations_killed`                                   |
| `mutations_survived`         | <!--fig:mutations_survived-->0<!--/fig-->                                                                 | `artifacts/mutation-kill.json` `mutations_survived`                                 |
| `mutations_registered`       | <!--fig:mutations_registered-->74<!--/fig-->                                                              | `scripts/mutation-kill.ts`, `id:` declarations in the registry                      |
...
registry cannot drift from the file it mutates. <!--fig:db_controls_declared-->72<!--/fig-->
checks, <!--fig:db_controls_killed-->72<!--/fig--> killed, <!--fig:db_controls_survived-->0<!--/fig-->
...
| `scripts/parse-test-summary.ts`                       | `MINIMUM_TESTS` at 646 under a docstring saying the floor is pinned to the measured run                 | the battery measures <!--fig:suite_tests-->829<!--/fig-->. The constant is raised rather than the docstring softened (§5.1)                                     |
