...
EXIST ACROSS R5, R6 AND R7, 6 OF THEM FAILED THEIR OWN EXACT-SHA RUN, AND EVERY ONE IS
...
**`065_appraisal_trade_pricing_desking.sql` is POST-FBL-020 work — FBL-120 (Appraisal,
Trade, Pricing and Desking), not part of this delivery.** It is listed here for the same
reason `061` through `064` are: this table is the one census of the migration chain on
disk, and its own account is
[`docs/FBL-120-DELIVERY-REPORT.md`](FBL-120-DELIVERY-REPORT.md). Over the immutable
`000`–`064` it adds the desk file opened from Release Train 4's one handed-on fact, the
trade unit and the versioned evidence about it, the rule book of taxes, fees, incentives,
valuations and policies — each naming its source, scope, interval and version, and none of
them overlapping, because `uq_desking_rules_no_overlap` is a GiST exclusion constraint —
the deterministic priced versions themselves, and the attributable manager decision that
freezes exactly one of them: fourteen tables, every one row-secured, registering
`desking_case`, `appraisal` and `desking_scenario` in BOTH resolvers. It is the first
migration in this chain to carry money, and every figure in it is an integer in minor
units. It takes ONE thing away: `064`'s `ck_desking_pre_fbl120`, the CHECK that pinned
`desking_handoffs.desking_status` to NOT_YET_AVAILABLE while the desk did not exist — which
is what `064`'s own header said the FBL-120 migration would do. It leaves `063`'s
`ck_attribution_pre_sale_revenue` exactly where it is: an approved scenario is a priced
proposal, not a sale.

**`066_deal_jacket_documents_esign.sql` is POST-FBL-020 work — FBL-140 (Deal Jacket,
Documents and E-Sign Evidence), not part of this delivery.** It is listed here for the same
reason `061` through `065` are: this table is the one census of the migration chain on
disk, and its own account is
[`docs/FBL-140-DELIVERY-REPORT.md`](FBL-140-DELIVERY-REPORT.md). Over the immutable
`000`–`065` it adds the deal jacket opened from exactly one manager-approved desking version,
the versioned and effective-dated document templates, requirements and retention policies
each naming its source — a template defaulting to `unapproved_sample` and refusing, by CHECK,
to call itself approved without an accountable approver — the immutable package versions
with their fields and content-addressed artifacts, the signing ceremony bound to one package
hash with its signers, consent, intent, signatures and append-only event ledger, the
short-lived artifact grants, and the legal-hold history: fifteen tables, every one
row-secured, registering `deal_jacket`, `jacket_package` and `signing_ceremony` in BOTH
resolvers. It grants the runtime no DELETE on any of them. It leaves `063`'s
`ck_attribution_pre_sale_revenue` exactly where it is: a signed package is evidence that
people signed documents, not a sale.
...
| `scripts/parse-test-summary.ts`              | declared floors **961 / 100**, above every minimum below                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `scripts/mutation-kill.ts`                   | **121 declared / 121 killed / 0 survived** — a COMPLETE run of the whole registry, every baseline green, the working tree intact after each. The artifact records WHEN it was taken and WHAT it measured in its own `generated_at`, `head` and `tree_dirty` fields; an earlier run that came back 43 / 1 on a red baseline is disclosed in §2.2 (S6a) rather than dropped                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `scripts/database-control-mutations.ts`      | **<!--fig:db_controls_declared-->98<!--/fig--> declared / <!--fig:db_controls_killed-->98<!--/fig--> killed / <!--fig:db_controls_survived-->0<!--/fig--> survived** — each §3 database control DROPPED from a copy of the migrated database, a NAMED test required to die, then restored and required to pass again (§6.1). `controls_filtered` is `false`, so a narrowed diagnostic run cannot stand in for the gate                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
**121 declared / 121 killed / 0 survived, 0 baselines red, 0 trees left dirty.** A report that
...
| `scripts/parse-test-summary.ts` | <!--fig:floor_tests-->961<!--/fig--> / <!--fig:floor_suites-->100<!--/fig-->                      | the DECLARED floor, pinned to what this revision actually measured                      |
...
| ---------------------------- | --------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `suite_tests`                | <!--fig:suite_tests-->961<!--/fig-->                                                                      | `artifacts/test-summary.json` `tests`                                               |
| `suite_suites`               | <!--fig:suite_suites-->100<!--/fig-->                                                                     | `artifacts/test-summary.json` `suites`                                              |
| `suite_passed`               | <!--fig:suite_passed-->961<!--/fig-->                                                                     | `artifacts/test-summary.json` `passed`                                              |
| `observed_ok`                | <!--fig:observed_ok-->1061<!--/fig-->                                                                     | `artifacts/test-summary.json` `observed_ok_lines`                                   |
| `observed_not_ok`            | <!--fig:observed_not_ok-->0<!--/fig-->                                                                    | `artifacts/test-summary.json` `observed_not_ok_lines`                               |
| `floor_tests`                | <!--fig:floor_tests-->961<!--/fig-->                                                                      | `scripts/parse-test-summary.ts` `MINIMUM_TESTS`                                     |
| `floor_suites`               | <!--fig:floor_suites-->100<!--/fig-->                                                                     | `scripts/parse-test-summary.ts` `MINIMUM_SUITES`                                    |
...
| `mutations_declared`         | <!--fig:mutations_declared-->121<!--/fig-->                                                               | `artifacts/mutation-kill.json` `mutations_total`                                    |
| `mutations_killed`           | <!--fig:mutations_killed-->121<!--/fig-->                                                                 | `artifacts/mutation-kill.json` `mutations_killed`                                   |
| `mutations_survived`         | <!--fig:mutations_survived-->0<!--/fig-->                                                                 | `artifacts/mutation-kill.json` `mutations_survived`                                 |
| `mutations_registered`       | <!--fig:mutations_registered-->121<!--/fig-->                                                             | `scripts/mutation-kill.ts`, `id:` declarations in the registry                      |
...
registry cannot drift from the file it mutates. <!--fig:db_controls_declared-->98<!--/fig-->
checks, <!--fig:db_controls_killed-->98<!--/fig--> killed, <!--fig:db_controls_survived-->0<!--/fig-->
...
| `scripts/parse-test-summary.ts`                       | `MINIMUM_TESTS` at 646 under a docstring saying the floor is pinned to the measured run                 | the battery measures <!--fig:suite_tests-->961<!--/fig-->. The constant is raised rather than the docstring softened (§5.1)                                     |
