...
3. Configure the **redirect URI** exactly as deployed:
   `https://<host>/auth/callback`. WorkOS matches exactly — no wildcards, no
   trailing-slash drift. Add the logout return URL too.
...
| `WORKOS_REDIRECT_URI`              | exactly the registered callback (https in production)    |
